# Don't Remove Credit @hero_botss
# Subscribe YouTube Channel For Amazing Bot @bots_repo
# Ask Doubt on telegram @hero_botss



from os import path, getenv

class Config:
    API_ID = int(getenv("API_ID", "22672907"))
    API_HASH = getenv("API_HASH", "0ff15ae2153bd8e03b48cb293010bc6a")
    BOT_TOKEN = getenv("BOT_TOKEN", "6615592453:AAEgKfReYuZns7Q-FQVFPITAqWT0q2l6_wU")
    FSUB = getenv("FSUB", "bots_repo")
    CHID = int(getenv("CHID", "-1002064960434"))
    SUDO = list(map(int, getenv("SUDO", "6287591671").split()))
    MONGO_URI = getenv("MONGO_URI", "mongodb+srv://infotechhero890:7c2qvHdJUYqTOaMa@cluster0.veojhex.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
    
cfg = Config()

# Don't Remove Credit @hero_botss
# Subscribe YouTube Channel For Amazing Bot @bots_repo
# Ask Doubt on telegram @hero_botss

